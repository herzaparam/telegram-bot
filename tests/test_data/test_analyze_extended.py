...
class TestAnalyzeStageStoreBacked:
    ...
    @pytest.mark.asyncio
    async def test_analyze_stage_stores_signals_from_all_engines(
        self, mock_stock_asset, sample_df
    ):
        """analyze_stage with mocked DB data creates 13 engine signals for stock."""
        ...
        with (
            ...
            patch("src.data.analyze._load_onchain_data", new_callable=AsyncMock, return_value=None),
            patch("src.data.analyze._load_github_data", new_callable=AsyncMock, return_value=None),
            patch("src.data.analyze._compute_correlation_data", new_callable=AsyncMock, return_value=None),
            ...
            # Stock asset should have 13 engines (Phase 3 + 8 + 9 + 10)
